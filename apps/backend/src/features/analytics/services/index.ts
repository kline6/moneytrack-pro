...
export const analyticsService = {
  async getDashboard(userId: string) {
    ...
    return {
      ...
      budget: (() => {
        const overall = budgetSummary.overall as any;
        const hasOverall = overall && overall.amount > 0;
        const budget = hasOverall ? overall.amount : budgetSummary.categoryTotalBudget;
        const spent = hasOverall ? overall.spent : budgetSummary.categoryTotalSpent;
        const remaining = hasOverall ? overall.remaining : budgetSummary.categoryTotalRemaining;
        return {
          totalBudget: budget,
          totalSpent: spent,
  ...
  async getSmartInsights(userId: string, viewType: 'day' | 'week' | 'month') {
    ...
    if (viewType === 'month') {
      ...
      const bsOverall = budgetSummary.overall as any;
      if (bsOverall && bsOverall.amount > 0) {
        ...
        const budgetPercent = bsOverall.percentage;
        if (timePercent < 50 && budgetPercent > 70) {
          budgetAlert = { level: 'warning', message: `本月仅过${timePercent}%，预算已消耗${budgetPercent}%，消费速度偏快` };
        } else if (budgetPercent > 100) {
          budgetAlert = { level: 'exceeded', message: `本月预算已超支${budgetPercent - 100}%` };
        }
      }
    }
    ...
    if (viewType === 'week') {
      ...
      if (diff > 0) {
        weekComparison = { message: `本周比上周多花${(diff / 100).toFixed(0)}元`, diff };
      } else if (diff < 0) {
        weekComparison = { message: `本周比上周少花${(Math.abs(diff) / 100).toFixed(0)}元`, diff };
      }
    }
    let dailyTrend = null;
